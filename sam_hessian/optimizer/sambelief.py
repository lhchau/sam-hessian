    ...
    def second_step(self, zero_grad=False):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if 'mean_grad_sq' not in param_state:
                    param_state['mean_grad_sq'] = torch.zeros_like(p, memory_format=torch.preserve_format)
                param_state['mean_grad_sq'].mul_(self.beta1).add_(p.grad, alpha=1-self.beta1)
                
                grad_residual = p.grad - param_state['mean_grad_sq']
                if 'var_grad_sq' not in param_state:
                    param_state['var_grad_sq'] = torch.zeros_like(p, memory_format=torch.preserve_format)
                param_state['var_grad_sq'].mul_(self.beta2).addcmul_(grad_residual, grad_residual, value=1-self.beta2)
                
                mask = param_state['var_grad_sq'] > self.threshold
