...
class MODIFIEDSAME(torch.optim.Optimizer):
    def __init__(self, params, rho=0.05, adaptive=False, condition=2, **kwargs):
        assert rho >= 0.0, f"Invalid rho, should be non-negative: {rho}"

        defaults = dict(rho=rho, adaptive=adaptive, **kwargs)
        super(MODIFIEDSAME, self).__init__(params, defaults)
        self.state['step'] = 0
        self.log_step = 176
        self.eps = 1e-8
    ...
    def second_step(self, zero_grad=False): 
        for group in self.param_groups:
            for p in group['params']:
                ...
                param_state['mask_ckpt1_second_step'] = ratio > 0
                    
                p.add_(param_state['e_w'] * self.condition)
    ...
    def third_step(self, zero_grad=False):
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                ratio = p.grad.div(param_state['first_grad'].add(1e-8))
                mask = torch.logical_and( param_state['mask_ckpt1_second_step'], ratio < 0 )
                d_p = p.grad.mul( torch.logical_not(mask) ) + param_state['first_grad'].mul( mask )
                
                p.sub_(param_state['e_w'] * (self.condition + 1))  # get back to "w" from "w + e(w)"
