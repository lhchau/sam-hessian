...
class SAMSLOWNOISY(torch.optim.Optimizer):
    def __init__(self, params, rho=0.05, adaptive=False, threshold=0.0005, condition=0.1, **kwargs):
        assert rho >= 0.0, f"Invalid rho, should be non-negative: {rho}"

        defaults = dict(rho=rho, adaptive=adaptive, **kwargs)
        super(SAMSLOWNOISY, self).__init__(params, defaults)
        self.state['step'] = 0
        self.log_step = 176
        self.total_para = 0
    ...
    def first_step(self, zero_grad=False):   
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if 'mean_grad_sq_old_g' not in param_state:
                    param_state['mean_grad_sq_old_g'] = torch.zeros_like(p, memory_format=torch.preserve_format)
                param_state['mean_grad_sq_old_g'].mul_(self.beta).addcmul_(p.grad, p.grad.conj(), value=1-self.beta)
                
                if 'var_grad_sq_old_g' not in param_state:
                    param_state['var_grad_sq_old_g'] = torch.zeros_like(p, memory_format=torch.preserve_format)
                param_state['var_grad_sq_old_g'].mul_(self.beta).add_(p.grad.mul( p.grad.conj() ).sub( param_state['mean_grad_sq_old_g'] ), alpha=1-self.beta )
    ...
    def second_step(self, zero_grad=False):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                mask = param_state['var_grad_sq_old_g'] > self.threshold
                d_p = torch.where( mask, p.grad.mul(self.condition), p.grad )
