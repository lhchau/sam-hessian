...
class OVERSAM(torch.optim.Optimizer):
    def __init__(self, params, rho=0.05, threshold=2, adaptive=False, **kwargs):
        assert rho >= 0.0, f"Invalid rho, should be non-negative: {rho}"

        defaults = dict(rho=rho, adaptive=adaptive, **kwargs)
        super(OVERSAM, self).__init__(params, defaults)
        self.state['step'] = 0
        self.log_step = 176
        self.total_para = 0
    ...
    def second_step(self, zero_grad=False):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                ratio.clamp_(-self.threshold, self.threshold)
