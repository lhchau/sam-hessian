...
class CUSTOMSAME2MUL34MUL(torch.optim.Optimizer):
    def __init__(self, params, rho=0.05, adaptive=False, condition1=1, condition2=1, **kwargs):
        assert rho >= 0.0, f"Invalid rho, should be non-negative: {rho}"

        defaults = dict(rho=rho, adaptive=adaptive, **kwargs)
        super(CUSTOMSAME2MUL34MUL, self).__init__(params, defaults)
        self.state['step'] = 0
        self.log_step = 176
        self.total_para = 0
        self.condition1, self.condition2 = condition1, condition2
    ...
    def second_step(self, zero_grad=False):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                mask2, mask34 = torch.logical_and( ratio > 0, ratio < 1), ratio < 0
                
                d_p = p.grad.mul( torch.logical_not( mask2 | mask34 )) + p.grad.mul(mask2).mul(self.condition1) + p.grad.mul(mask34).mul(self.condition2)
