...
class GEOSAM(torch.optim.Optimizer):
    def __init__(self, params, rho=0.05, adaptive=False, **kwargs):
        assert rho >= 0.0, f"Invalid rho, should be non-negative: {rho}"

        defaults = dict(rho=rho, adaptive=adaptive, **kwargs)
        super(GEOSAM, self).__init__(params, defaults)
        self.state['step'] = 0
        self.log_step = 176
        self.eps = 1e-8
        self.total_para = 0
    ...
    def third_step(self, zero_grad=False):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                d_p = param_state["second_grad"] + 0.1 * (param_state["second_grad"] + p.grad - 2 * param_state["first_grad"]) + 0.01 * (param_state["second_grad"] - p.grad)
                if step % self.log_step == 0:
                    ratio = p.grad.div(param_state['first_grad'].add(1e-8))
                ...
                p.add_(param_state['e_w'])  # get back to "w" from "w + e(w)"
