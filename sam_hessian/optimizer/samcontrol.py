...
class SAMCONTROL(torch.optim.Optimizer):
    def __init__(self, params, rho=0.05, adaptive=False, condition=2, threshold=2, **kwargs):
        assert rho >= 0.0, f"Invalid rho, should be non-negative: {rho}"

        defaults = dict(rho=rho, adaptive=adaptive, **kwargs)
        super(SAMCONTROL, self).__init__(params, defaults)
        self.state['step'] = 0
        self.log_step = 176
        self.eps = 1e-8
