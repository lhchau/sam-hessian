...
class CLIPSAMNEG(torch.optim.Optimizer):
    def __init__(self, params, rho=0.05, condition=10, adaptive=False, **kwargs):
        assert rho >= 0.0, f"Invalid rho, should be non-negative: {rho}"

        defaults = dict(rho=rho, adaptive=adaptive, **kwargs)
        super(CLIPSAMNEG, self).__init__(params, defaults)
        self.state['step'] = 0
        self.log_step = 176
        self.total_para = 0
    ...
    def second_step(self, zero_grad=False):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                mask_neg = param_state['first_grad'].mul(p.grad).lt(0)  
                threshold = param_state['first_grad'].abs().mul(self.condition)  
                abs_grad = p.grad.abs()  
                clamp_grad = torch.min(abs_grad, threshold) 

                d_p = torch.where(mask_neg, clamp_grad.mul(p.grad.sign()), p.grad)
                
                if step % self.log_step == 0:
                    ratio = p.grad.div( param_state['first_grad'] + 1e-8 )
