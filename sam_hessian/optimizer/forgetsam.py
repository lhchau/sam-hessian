...
class FORGETSAM(torch.optim.Optimizer):
    def __init__(self, params, rho=0.05, adaptive=False, condition=0.5, **kwargs):
        assert rho >= 0.0, f"Invalid rho, should be non-negative: {rho}"

        defaults = dict(rho=rho, adaptive=adaptive, **kwargs)
        super(FORGETSAM, self).__init__(params, defaults)
        self.state['step'] = 0
        self.log_step = 176
    ...
    def second_step(self, zero_grad=False):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                mask_forget = param_state['ratio'] < 0
                param_state['forget_grad'] = p.grad.mul( mask_forget )
                param_state['learn_grad'] = p.grad.mul( torch.logical_not( mask_forget ) )
        self.forget_grad_norm = self._grad_norm('forget_grad')
        self.learn_grad_norm = self._grad_norm('learn_grad')
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                d_p = param_state['learn_grad'] + param_state['forget_grad'].mul( self.learn_grad_norm * self.condition / self.forget_grad_norm )
                
                if step % self.log_step == 0:
                    ratio = param_state['ratio']
